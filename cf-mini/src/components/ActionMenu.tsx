...
export default function ActionMenu({
  ...
  const iconCls = isFullMenu ? "h-[15px] w-[15px] md:h-[19px] md:w-[19px]" : "h-[19px] w-[19px]";
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {(open || closing) && (
        <div
          className={`absolute right-full top-1/2 z-20 mr-1.5 flex h-[38px] origin-right -translate-y-1/2 items-center overflow-hidden rounded-[7px] bg-[#4c4c4c] text-white shadow-lg ${closing ? "animate-pop-out" : "animate-pop-in"}`}
        >
          {onLike && (
            <button type="button" onClick={() => { onLike(); closeMenu(); }} className={itemCls}>
              <Heart className={`${iconCls} ${liked ? "text-red-500" : ""}`} fill={liked ? "currentColor" : "none"} strokeWidth={1.8} />
              <span>{liked ? "取消" : "赞"}</span>
            </button>
          )}
          {onLike && onComment && <Divider />}
          {onComment && (
            <button type="button" onClick={() => { onComment(); closeMenu(); }} className={itemCls}>
          ...
          {onPin && (
            <button type="button" onClick={() => { onPin(); closeMenu(); }} className={itemCls}>
              {pinned ? <PinOff className={iconCls} strokeWidth={1.8} /> : <Pin className={iconCls} strokeWidth={1.8} />}
              <span>{pinned ? "取消置顶" : "置顶"}</span>
            </button>
          )}
          {onEdit && (onLike || onComment || onPin) && <Divider />}
          {onEdit && (
            <button type="button" onClick={() => { onEdit(); closeMenu(); }} className={itemCls}>
          ...
          {onDelete && (
            <button type="button" onClick={() => { onDelete(); closeMenu(); }} className={`${itemCls} text-red-300`}>
